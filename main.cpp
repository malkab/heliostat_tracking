...
*/
    ...
    vec3d r = myTracker.secondary.getTransform(myTarget->angles.y).transformPoint(myTracker.facet.shift);
    r = myTracker.primary.getTransform(myTarget->angles.x).transformPoint(r);
    std::cout << "Facet reference point location: " << r << std::endl;

    //Heliostat normal
    vec3d n = myTracker.secondary.getTransform(myTarget->angles.y*gcf::degree).transformNormal(myTracker.facet.normal);
    n = myTracker.primary.getTransform(myTarget->angles.x*gcf::degree).transformPoint(n);
